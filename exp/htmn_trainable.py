...
class HTMNTrainable(tune.Trainable):
    ...
    def setup(self, config):
        ...
        if self.holdout > 0:
            ...
            self.tr_idx = np.arange(len(dataset))
            self.tr_ld = DataLoader(dataset, batch_size=config['batch_size'], shuffle=True, collate_fn=trees_collate_fn, drop_last=len(self.tr_idx) % config['batch_size'] == 1)
